/***************************************************************************
 *   Copyright (c) 2015-2019 Microsemi Corporation (A Microchip company)   *
...
FLASH_BANK_COMMAND_HANDLER(microsemi_smartfusion_envm_flash_bank_command)
{
    ...
    {
        LOG_ERROR("invalid envm base address (0x%08x) and/or size (0x%08x) specified", (uint32_t)(bank->base), bank->size);
        return ERROR_FAIL;
    }
...
)
{
    ...
    if (target_read_u32(bank->target, ENVM_CR_SYSREG, &envm_cr_old) == ERROR_OK)
    {
        ...
        if (target_write_u32(bank->target, ENVM_CR_SYSREG, envm_cr_new) != ERROR_OK)
        {
    ...
    LOG_INFO("Microsemi SmartFusion eNVM - writing %d (0x%x) bytes to address 0x%08x (. = 1024 bytes)", count, count, (uint32_t)(bank->base + offset));
    ...
    while ((count > 0) && (ENVM_SUCCESS == status))
    {
        ...
        if (memcmp(new_page, old_page, ENVM_PAGE_SIZE) != 0)
        {
            ...
            if (target_write_u32(bank->target, ENVM_CONTROL_REG, command) != ERROR_OK)
            {
                status = ENVM_TARGET_ACCESS_ERROR;
                continue;
            }
            ...
            do {
                ...
                {
                    ready = ((envm_status & g_aenvm_block[envm_block_id].busy) == 0);
                }
                else
                {
                    ready = 0;
        ...
        /* Yield to main loop every page */
        keep_alive();

        /* Log progress every 1kBytes (8 x 128 pages) written */
        if (((++progress) % 8) == 0)
        {
            LOG_USER_N(".");
        }
    ...
    if (NULL != psz_error)
    {
        LOG_ERROR("eNVM write failed at address 0x%08x/page %d - %s",
            (uint32_t)(envm_write_address - ENVM_BASE_ADDRESS + bank->base),
            (int32_t)(ENVM_PAGE_NUM(envm_write_address - ENVM_BASE_ADDRESS + bank->base)),
            psz_error);
    }
...
const struct flash_driver microsemi_smartfusion_envm_flash = {
