...
FLASH_BANK_COMMAND_HANDLER(microsemi_fusion_coreahbnvm_flash_bank_command)
{
    ...
    {
        LOG_ERROR("invalid envm base address ('0x%08x') and/or size ('0x%08x') specified",
...
)
{
    ...
    do {
        ...
        if (target_read_u8(bank->target, page_start_addr, &status_reg) != ERROR_OK)
        {
...
)
{
    ...
    while ((count > 0) && (ENVM_SUCCESS == status))
    {
        ...
        if (memcmp(new_page, old_page, ENVM_PAGE_SIZE) != 0)
        {
            ...
            if (microsemi_fusion_coreahbnvm_wait(bank, page_start_addr) != ENVM_SUCCESS)
            {
                continue;
            }
...
const struct flash_driver microsemi_fusion_coreahbnvm_flash = {
