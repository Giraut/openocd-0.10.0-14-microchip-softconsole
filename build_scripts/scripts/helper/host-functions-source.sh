...
function host_common()
{
  ...
  CONTAINER_BUILD_SCRIPT_REL_PATH="build_scripts/scripts/${CONTAINER_SCRIPT_NAME}"
  ...
  echo "skipping the build source copy to Work folder"
  echo "is the ${HOST_WORK_FOLDER_PATH}/build_scripts present? use it!"

  if [[ -d "${HOST_WORK_FOLDER_PATH}/build_scripts" ]]
  then
    echo "build_scripts is present where it is expected, continuing..."
  else
    echo "Scripts not found in ${HOST_WORK_FOLDER_PATH}/build_scripts and therefore aborting!"
    exit 1
  fi
  #copy_build_git
}
...
function host_prepare_prerequisites() 
{
  ...
  HOST_DEFINES_SCRIPT_PATH="${HOST_WORK_FOLDER_PATH}/build_scripts/scripts/host-defs-source.sh"
