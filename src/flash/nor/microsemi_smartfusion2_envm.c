...
)
{
    ...
    LOG_INFO("Microsemi SmartFusion2 eNVM - writing %d (0x%x) bytes to address 0x%08x (. = 1024 bytes)",
        count, count, (uint32_t)(bank->base + offset));
    ...
    if (NULL != psz_error) 
    {
        LOG_ERROR("eNVM write failed at address 0x%08x/page %d - %s",
            (uint32_t)(bank->base + offset + count - remaining_length),
...
const struct flash_driver microsemi_smartfusion2_envm_flash = {
...
)
{
    ...
    *pstatus = ENVM_SUCCESS;
    ...
    /* Which eNVM block? */
    envm_block_id = (offset < ENVM_BLOCK_SIZE) ? envm_block_0 : envm_block_1;

    /* Fill page WD buffer */
    *pstatus = envm_fill_page_buffer(bank, offset, length_written, pdata, envm_block_id, &f_modified);
