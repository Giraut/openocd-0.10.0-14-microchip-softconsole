...
static int microsemi_flashpro_speed(int speed)
{
	...
	else if (strcmp(sg_flashpro.info.type, "FlashPro6") == 0)
	{
		/* TODO: what's the "best" default speed? What does Libero/FPExpress select? */
		if (speed == -1) speed = 8000000;

		if ((speed < 1000000) || (speed > 20000000) || ((speed % 1000000) != 0))
		{
			LOG_ERROR("Invalid speed %dkHz specified - FlashPro6 speed must be a multiple of 1MHz between 1MHz and 20MHz specified in kHz, -1 will set it to 8MHz", speed / HZ_PER_KHZ);
			retval = ERROR_JTAG_DEVICE_ERROR;
		}
	}
	else
...
COMMAND_HANDLER(handle_microsemi_flashpro_port_command)
{
	...
	command_print(CMD, "microsemi_flashpro port %s", sg_flashpro.sz_port);
	return ERROR_OK;
}
...
COMMAND_HANDLER(handle_microsemi_flashpro_tunnel_jtag_via_ujtag_command)
{
	...
	command_print(CMD, "microsemi_flashpro tunnel_jtag_via_ujtag %s", n->name);
	return ERROR_OK;
}
